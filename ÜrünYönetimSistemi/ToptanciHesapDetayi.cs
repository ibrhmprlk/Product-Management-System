...
{
    public partial class ToptanciHesapDetayi : Form
    {
        ...
        private void TariheGoreFiltrele()
        {
            ...
            if (borcTablosu != null && borcTablosu.Columns.Contains("Tarih"))
            {
        ...
        private void ToptanciHesapDetayi_Load(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection baglan = new OleDbConnection(baglantiDizesi))
            {
                try
                {
                    ...
                    // textBox3’e Toptancilar’dan ToplamBorc’u direkt çek
                    using (OleDbCommand cmd = new OleDbCommand("SELECT ToplamBorc FROM Toptancilar WHERE GsmTelefon = ?", baglan))
                    {
                        ...
                        if (result != null && result != DBNull.Value)
                        {
                            ...
                            string borcStr = result.ToString();

                            // Noktayı virgüle çevirerek dönüştürmeyi dene
                            if (decimal.TryParse(borcStr.Replace(".", ","), out toplamBorc))
                            {
                            ...
                            {
                                // Dönüşüm başarısız olursa, hatalı da olsa orijinal değeri göster
                                textBox3.Text = borcStr;
                                textBox5.Text = borcStr;
                                MessageBox.Show("Borç değeri dönüştürülemedi: " + borcStr, "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                        }
                        else
                        {
                            textBox3.Text = "0.00";
                            textBox5.Text = "0.00";
                            MessageBox.Show("Bu GsmTelefon için toptancı kaydı bulunamadı.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                    }

                    // dataGridView1’i doldur, ama textBox3’e etkisi olmasın
                    DataTable tablo = new DataTable();
                    string query = @"
                SELECT [Tarih/Saat] AS Tarih, Aciklama AS [İşlem Türü], EklenenTutar AS [İşlem Tutarı], EskiBorc AS [Önceki Bakiye], ToplamBorc AS [Kalan Borç]
                FROM BorcEkleme 
                WHERE GsmTelefon = ?
                UNION ALL
                SELECT [Tarih/Saat] AS Tarih, Aciklama AS [İşlem Türü], -OdenenTutar AS [İşlem Tutarı], EskiBorc AS [Önceki Bakiye], 
                ...
                ORDER BY Tarih ASC";
                    ...
                    adapter.SelectCommand.Parameters.AddWithValue("@GsmTelefon", GsmTelefon);
                    adapter.SelectCommand.Parameters.AddWithValue("@GsmTelefon", GsmTelefon);
                    adapter.Fill(tablo);
                    ...
                    {
                        dataGridView1.Columns["Tarih"].HeaderText = "Tarih / Saat";
                        ...
                        dataGridView1.Columns["İşlem Tutarı"].DefaultCellStyle.Format = "N2";
                        dataGridView1.Columns["Önceki Bakiye"].DefaultCellStyle.Format = "N2";
                        dataGridView1.Columns["Kalan Borç"].DefaultCellStyle.Format = "N2";
            ...
            UrunAlislariniDataGridDoldur();
            UrunIadeleriniDataGridDoldur();
        ...
        private void UrunAlislariniDataGridDoldur()
        {
            ...
            using (OleDbConnection baglan = new OleDbConnection(baglantiDizesi))
            {
                try
                {
                    ...
                    // GlobalAyarlar sınıfından değeri alıyor
                    string fiyatSutunu = GlobalAyarlar.KdvDahilGoster ? "Alis_Fiyati" : "Alis_Fiyati2";

                    string urunSorgusu = $@"
                SELECT Tarih, 'Ürün Alışı' AS IslemTuru, Ürün_Adi, {fiyatSutunu} AS BirimFiyati,Miktar, IslemTuru AS OdemeSekli 
                FROM ÜrünGirişi 
                WHERE GsmTelefon = @GsmTelefon 
                ORDER BY Tarih ASC";

                    OleDbDataAdapter urunAdapter = new OleDbDataAdapter(urunSorgusu, baglan);
                    urunAdapter.SelectCommand.Parameters.AddWithValue("@GsmTelefon", GsmTelefon);
                    urunAdapter.Fill(urunTablosu);

                    urunTablosu.Columns.Add("ToplamTutar", typeof(decimal));

                    foreach (DataRow row in urunTablosu.Rows)
                    {
                        if (decimal.TryParse(row["BirimFiyati"].ToString().Replace('.', ','), out decimal birimFiyati) &&
                            decimal.TryParse(row["Miktar"].ToString(), out decimal miktar))
                        {
                            row["ToplamTutar"] = birimFiyati * miktar;
                        }
                        else
                        {
                            row["ToplamTutar"] = 0;
                    ...
                    dataGridView2.DataSource = urunTablosu;

                    if (dataGridView2.Columns.Count >= 7)
                    {
                        // Sütun başlıkları ve sıraları
                        dataGridView2.Columns["Tarih"].HeaderText = "Tarih / Saat";
                        dataGridView2.Columns["IslemTuru"].HeaderText = "İşlem Türü";
                        dataGridView2.Columns["Ürün_Adi"].HeaderText = "Ürün Adı";
                        dataGridView2.Columns["BirimFiyati"].HeaderText = GlobalAyarlar.KdvDahilGoster ? "Birim Fiyatı (KDV Dahil)" : "Birim Fiyatı (KDV Hariç)";
                        dataGridView2.Columns["Miktar"].HeaderText = "Miktar";
                        dataGridView2.Columns["ToplamTutar"].HeaderText = "Toplam Tutar";
                        dataGridView2.Columns["OdemeSekli"].HeaderText = "Ödeme Şekli";

                        dataGridView2.Columns["Tarih"].DisplayIndex = 0;
                        dataGridView2.Columns["IslemTuru"].DisplayIndex = 1;
                        dataGridView2.Columns["Ürün_Adi"].DisplayIndex = 2;
                        dataGridView2.Columns["BirimFiyati"].DisplayIndex = 3;
                        dataGridView2.Columns["Miktar"].DisplayIndex = 4;
                        dataGridView2.Columns["ToplamTutar"].DisplayIndex = 5;
                        dataGridView2.Columns["OdemeSekli"].DisplayIndex = 6;

                        // Hücre biçimlendirmeleri
                        dataGridView2.Columns["BirimFiyati"].DefaultCellStyle.Format = "C2";
                        dataGridView2.Columns["ToplamTutar"].DefaultCellStyle.Format = "C2";
                        dataGridView2.Columns["BirimFiyati"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                        ...
                        dataGridView2.Columns["IslemTuru"].Width = 150;
                        dataGridView2.Columns["Ürün_Adi"].Width = 150;
                        dataGridView2.Columns["BirimFiyati"].Width = 135;
                        dataGridView2.Columns["Miktar"].Width = 130;
                        dataGridView2.Columns["ToplamTutar"].Width = 110;
                        dataGridView2.Columns["OdemeSekli"].Width = 110;
                ...
                {
                    MessageBox.Show("Ürün alışı verileri yüklenirken hata: " + ex.Message, "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        private void UrunIadeleriniDataGridDoldur()
        {
            // Seçilen GsmTelefon boşsa çık
            ...
            using (OleDbConnection baglan = new OleDbConnection(baglantiDizesi))
            {
                try
                {
                    ...
                    string iadeSorgusu = @"
        SELECT Tarih, Saat, ID, Barkod_No, Ürün_Adi, Aciklama, Stok_Miktari, IadeEdilenMiktar, ToplamTutar
        FROM UrunIade
        WHERE GsmTelefon = @GsmTelefon
        ORDER BY ID ASC";
                    ...
                    dataGridView3.Columns["IadeEdilenMiktar"].HeaderText = "İade Edilen Miktar";
                    dataGridView3.Columns["IadeEdilenMiktar"].DisplayIndex = 5;
                    ...
                    dataGridView3.Columns["IadeEdilenMiktar"].Width = 130;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string fileName = "ToptanciBorcDetayi_" + textBox1.Text + ".xlsx";
                ...
                {
                    var worksheet = workbook.Worksheets.Add("Toptancı Borç Detayı");
                    int currentRow = 1;
                    ...
                    // Üst bilgiler
                    worksheet.Cell(currentRow, 1).Value = "Toptancı Adı:";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                SaveFileDialog sfd = new SaveFileDialog();
                sfd.Filter = "Excel Dosyası|*.xlsx";
                string fileName = "UrunAlisDetayi_" + textBox1.Text + ".xlsx";
                sfd.FileName = fileName;
                ...
                using (var workbook = new XLWorkbook())
                {
                    var worksheet = workbook.Worksheets.Add("Ürün Alış Detayı");
                    int currentRow = 1;
                    ...
                    worksheet.Cell(currentRow, 1).Value = "Ürünlerin Toplam Fiyatı:";
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    worksheet.Range(currentRow, 1, currentRow, 7).Merge();
                    worksheet.Cell(currentRow, 1).Value = $"Toptancı Adı : {textBox1.Text}";
                    worksheet.Cell(currentRow, 1).Style.Font.Bold = true;
