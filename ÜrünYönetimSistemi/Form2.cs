...
namespace ÜrünYönetimSistemi
{
    public partial class Form2 : Form
    {
        ...
        private void DisableAllButtons()
        {
            ...
            button11.Enabled = true;  // Çıkış her zaman enabled
        ...
        {
            // Yükleme formunu aç
            frmverilerAktarılıyor aktarForm = new frmverilerAktarılıyor();
            aktarForm.Show();
            Application.DoEvents(); // Formun hemen çizilmesini sağlar

            bool yedeklemeBasarili = false;
            Exception yedeklemeHatasi = null;

            // Asenkron olarak yedekleme işlemini çalıştır
            await Task.Run(() =>
            {
                ...
                string hedefDosya = Path.Combine(yedekKlasoru, $"Veritabani_yedek_{zamanDamgasi}.accdb");
                ...
                try
                {
                    ...
                    if (File.Exists(kaynakDosya))
                    {
                        ...
                        yedeklemeBasarili = true;
                    }
                    else
                    {
                        throw new FileNotFoundException($"Kaynak dosya bulunamadı: {kaynakDosya}");
                    }
                }
                catch (Exception ex)
                {
            ...
            if (aktarForm != null && !aktarForm.IsDisposed)
            {
                aktarForm.Close();
            }

            // Yedekleme sonucu (kullanıcı istemediği için MessageBox'lar kaldırıldı)
            if (yedeklemeHatasi != null)
            {
                // Yalnızca arka planda bir hata olursa, loglama amaçlı (gerekirse açılabilir)
                // MessageBox.Show("Yedekleme sırasında kritik bir hata oluştu: " + yedeklemeHatasi.Message, "Kritik Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            // Hata olsun olmasın uygulamayı kapat
            Application.Exit();
        }
