...
namespace ÜrünYönetimSistemi
{
    public partial class Ürün_Girişi : Form
    {
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            ...
            // 6️⃣ Toptancı kontrolü
            string secilenGsmTelefon = ""; // secilenGsmTelefon değişkeni tanımlı değilse burada varsayılan değer atanabilir
            if (comboBox2.Text != "Toptancı Adını Belirtmek istemiyorum" && string.IsNullOrWhiteSpace(secilenGsmTelefon))
            {
                MessageBox.Show("Toptancı telefon numarası bulunamadı. Lütfen geçerli bir toptancı seçiniz.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            try
            {
                using (OleDbConnection baglan = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + Application.StartupPath + "\\ÜrünYönetimSistemi.accdb"))
                {
                    ...
                    // 10️⃣ Toptancı borç mantığı (eski + yeni + değişiklik)
                    ...
                    {
                        // Eski toptancıdan düş
                        if (!string.IsNullOrWhiteSpace(eskiGsmTelefon) && eskiGsmTelefon != secilenGsmTelefon)
                        {
                        ...
                        // Yeni toptancıya ekle veya varsa güncelle
                        ...
                        using (OleDbCommand cmd = new OleDbCommand(selectYeni, baglan))
                        {
                            ...
                            if (res != DBNull.Value && res != null)
                            {
                                string borcStr = res.ToString().Trim();
                                if (borcStr.Contains(",")) borcStr = borcStr.Replace(",", ".");
                                decimal.TryParse(borcStr, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out yeniToplamBorcYeni);
                            }
                        }
                        ...
                        {
                            cmd.Parameters.AddWithValue("@ToplamBorc", yeniToplamBorcYeni.ToString(System.Globalization.CultureInfo.InvariantCulture));
                            cmd.Parameters.AddWithValue("@GsmTelefon", secilenGsmTelefon);
                            ...
                            if (affected == 0)
                            {
                                ...
                                using (OleDbCommand cmdInsert = new OleDbCommand(insertYeni, baglan))
                                {
                                    ...
                                    cmdInsert.Parameters.AddWithValue("@ToplamBorc", yeniToplamBorcYeni.ToString(System.Globalization.CultureInfo.InvariantCulture));
                                    cmdInsert.ExecuteNonQuery();
                                }
                            }
                        }
                    }
                    else if (comboBox2.Text == "Toptancı Adını Belirtmek istemiyorum" && !string.IsNullOrWhiteSpace(eskiGsmTelefon))
                    {
                        decimal mevcutBorc = 0;
                        string selectBorcQuery = "SELECT ToplamBorc FROM Toptancilar WHERE GsmTelefon=@GsmTelefon";
                        using (OleDbCommand cmd = new OleDbCommand(selectBorcQuery, baglan))
                        {
                            cmd.Parameters.AddWithValue("@GsmTelefon", eskiGsmTelefon);
                            object res = cmd.ExecuteScalar();
                            if (res != DBNull.Value && res != null)
                            {
                                string borcStr = res.ToString().Trim();
                                if (borcStr.Contains(",")) borcStr = borcStr.Replace(",", ".");
                                decimal.TryParse(borcStr, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out mevcutBorc);
                            }
                        }
                        decimal yeniToplamBorc = mevcutBorc - eskiBorc;
                        if (yeniToplamBorc < 0) yeniToplamBorc = 0;
                        string updateBorc = "UPDATE Toptancilar SET ToplamBorc=@ToplamBorc WHERE GsmTelefon=@GsmTelefon";
                        using (OleDbCommand cmd = new OleDbCommand(updateBorc, baglan))
                        {
                            cmd.Parameters.AddWithValue("@ToplamBorc", yeniToplamBorc.ToString(System.Globalization.CultureInfo.InvariantCulture));
                            cmd.Parameters.AddWithValue("@GsmTelefon", eskiGsmTelefon);
                            cmd.ExecuteNonQuery();
                        }
                    }
