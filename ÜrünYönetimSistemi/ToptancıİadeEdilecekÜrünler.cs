...
namespace ÜrünYönetimSistemi
{
    public partial class ToptancıİadeEdilecekÜrünler : Form
    {
        ...
        private bool UrunIadeEtVeBorcGuncelle(OleDbConnection baglan, DataGridViewRow selectedRow, decimal iadeMiktari, decimal birimFiyati, out string hataMesaji)
        {
            hataMesaji = "";
            try
            {
                ...
                {
                    hataMesaji = $"'{selectedRow.Cells["Ürün_Adi"].Value}' için stok yetersiz. Maksimum iade miktarı: {mevcutStok}";
                    return false;
                }

                // Toplam tutar küsüratlı olarak hesaplanıyor
                decimal iadeTutari = Math.Round(iadeMiktari * birimFiyati, 2);
                decimal mevcutBorc = 0;
                // GSM numarasına göre borç sorgula
                ...
                {
                    cmdSelect.Parameters.AddWithValue("@GsmTelefon", toptanciGsmTelefon);
                    object result = cmdSelect.ExecuteScalar();
                    if (result != DBNull.Value && result != null)
                    {
                        // Mevcut borcu küsüratlı olarak al
                        decimal.TryParse(result.ToString().Replace(",", "."), NumberStyles.Any, CultureInfo.InvariantCulture, out mevcutBorc);
                    }
                }

                if (mevcutBorc - iadeTutari < 0)
                {
                    hataMesaji = $"'{selectedRow.Cells["Ürün_Adi"].Value}' için iade tutarı mevcut borcu eksiye düşürecek!";
                    return false;
                }
                ...
                // GSM numarasına göre borç güncelle
                string borcGuncellemeSorgusu = "UPDATE Toptancilar SET ToplamBorc = ToplamBorc - @iadeTutari WHERE GsmTelefon = @gsmTelefon";
                using (OleDbCommand borcKomut = new OleDbCommand(borcGuncellemeSorgusu, baglan))
                {
                    borcKomut.Parameters.AddWithValue("@iadeTutari", iadeTutari);
                    borcKomut.Parameters.AddWithValue("@gsmTelefon", toptanciGsmTelefon);
                    borcKomut.ExecuteNonQuery();
                }
            ...
            {
                hataMesaji = $"Hata oluştu: {ex.Message}";
                return false;
            }
        }
...
ORDER BY Ürün_Adi ASC";
        ...
        private void dataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows.Count == 1)
            {
                ...
                textBox1.Text = satir.Cells["Miktar"].Value?.ToString();
            }
            else
            {
                textBox1.Clear();
